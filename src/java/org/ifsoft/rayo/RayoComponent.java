...
import org.jivesoftware.openfire.plugin.ofswitch.OfSwitchPlugin;
...
public class RayoComponent extends AbstractComponent
{
    ...
    private static final String sipDomain = JiveGlobals.getProperty("freeswitch.sip.hostname", OfSwitchPlugin.self.getIpAddress());
    private static final String sipGateway = JiveGlobals.getProperty("freeswitch.sip.gateway", OfSwitchPlugin.self.getIpAddress());

    private Map<String, CallParticipant> callList;

    public static RayoComponent self;

    private OfSwitchPlugin plugin;
    ...
    {
        public static final String PBX_COUNTRY_CODE              = "pbx.country.code";
        public static final String PBX_ACCESS_DIGITS             = "pbx.access.digits";
        public static final String AREA_CODE                     = "pbx.area.code";
        public static final String PBX_NUMBER_LENGTH             = "pbx.number.length";
        public static final String PBX_NUMBER_PREFIX             = "pbx.number.prefix";
        public static final String PBX_LOCAL_PHONE_NUMBERLENGTH  = "pbx.local.phone.numberlength";
        public static final String PBX_AREA_TRUNK_CODE           = "pbx.area.trunk.code";
        public static final String PBX_USE_IDD                   = "pbx.idd.format";

        public static final String PBX_FWD_CODE_PREFIX           = "pbx.fwd.code.prefix";
        public static final String PBX_FWD_CODE_SUFFIX           = "pbx.fwd.code.suffix";
        public static final String PBX_FWD_CODE_CANCEL           = "pbx.fwd.code.cancel";

        private Properties() {}
    }


    public RayoComponent(final OfSwitchPlugin plugin)
    {
        self = this;
        this.plugin = plugin;
    }
    ...
    public void doStart()
    {
        ...
        callList = new ConcurrentHashMap<String, CallParticipant>();
    }
        ...
        try {
            ...
            if (RAYO_CORE.equals(namespace)) {
                ...
                if (object instanceof JoinCommand) {
                    ...
                    reply = handleRedirectCommand((RedirectCommand) object, iq);
    ...
    private IQ handleHoldCommand(Object object, IQ iq)
    {
        ...
        if (callId != null)
        {
            Log.info("RayoComponent handleHoldCommand " + iq.getFrom() + " " + callId);

            OfSwitchPlugin.self.sendAsyncFWCommand("uuid_hold toggle " + callId);
...
*/
    ...
    private IQ handleDialCommand(DialCommand command, IQ iq)
    {
        ...
        if (join != null)
        {
            ...
            cp.setPhoneNumber(from);
            cp.setToPhoneNumber(to);
            cp.setName(calledName);
            cp.setHeaders(headers);

            reply = doMakeCall(JID.escapeNode(handsetId), cp, reply);
        }

        return reply;
    }

    private IQ handleRedirectCommand(RedirectCommand command, IQ iq)
    {
        IQ reply = IQ.createResultIQ(iq);
        String callId = iq.getTo().getNode();
        String destination = command.getTo().toString();

        if (callId != null)
        {
            Log.info("RayoComponent handleRedirectCommand " + iq.getFrom() + " " + callId + " " + destination);

            OfSwitchPlugin.self.sendAsyncFWCommand("uuid_transfer " + callId + " -bleg " + destination);
    ...
    private IQ handleHangupCommand(HangupCommand command, IQ iq)
    {
        ...
        if (callId != null)
        {
            ...
            OfSwitchPlugin.self.sendAsyncFWCommand("uuid_kill " + callId);
    ...
    private IQ doMakeCall(String handsetId, CallParticipant cp, IQ reply)
    {
        Log.info("RayoComponent doMakeCall " + handsetId + "\n" + cp);

        try {
            String callId = "rayo-" + System.currentTimeMillis();
            String cliNumber = cp.getPhoneNumber();
            String cliName = cp.getDisplayName();
            ...
            String destination = OfSwitchPlugin.self.makeDialableNumber(cp.getToPhoneNumber());

            if (destination != null)
            {
                callList.put(callId, cp);
                cp.setCallId(callId);

                String command = "originate {presence_id=" + cliNumber + "@" + sipDomain + ",origination_caller_id_name='" + destination + "',origination_caller_id_number='" + destination + "',sip_from_user='" + cliNumber + "',origination_uuid=" + callId + "}[sip_invite_params=intercom=true,sip_h_Call-Info=<sip:" + sipDomain + ">;answer-after=0,sip_auto_answer=true]user/" + cliNumber + " " + destination;
                String error = OfSwitchPlugin.self.sendAsyncFWCommand(command);
                ...
                if (error != null)
                {
                    ...
                    Log.error("doMakeCall failed " + error);
                    reply.setError(PacketError.Condition.not_allowed);
                }

            } else {
                Log.error("doMakeCall empty destination " + handsetId);
                reply.setError(PacketError.Condition.not_allowed);
            }

        } catch (Exception e) {
            Log.error("doMakeCall", e);
            reply.setError(PacketError.Condition.internal_server_error);
        }
...
*/
    ...
    public void callStateEvent(String eventName, Map<String, String> headers)
    {
        Log.debug("callStateEvent \n" + headers);
        ...
        {
            CallParticipant callParticipant = callList.get(callId);

            if (callParticipant != null)
            {
                Log.info("CHANNEL_CALLSTATE " + callState + " " + callId + " " + source + " " + destination + " " + callDirection + " " + origCallState + "\n" + callParticipant);

                Presence presence = new Presence();
                presence.setFrom(callId + "@" + getDomain());
                presence.setTo(callParticipant.getCallOwner());

                Map<String, String> rayoHeaders = callParticipant.getHeaders();

                rayoHeaders.put("call_owner", callParticipant.getCallOwner());
                rayoHeaders.put("call_protocol", callParticipant.getProtocol());
                rayoHeaders.put("mixer_name", "default");
                rayoHeaders.put("group_name", "default");

                if ("HANGUP".equals(callState))
                {
                    callList.remove(callId);
                    rayoHeaders.put("call_action", "leave");
                    presence.getElement().add(rayoProvider.toXML(new EndEvent(null, EndEvent.Reason.valueOf("HANGUP"), rayoHeaders)));
                    sendPacket(presence);
                }
                else
                ...
                {
                    rayoHeaders.put("call_action", "join");
                    setAnsweredState(presence, false, rayoHeaders);
                    sendPacket(presence);
                }
                else

                if ("DOWN".equals(origCallState) && "RINGING".equals(callState) && "outbound".equals(callDirection))
                {
                    setRingingState(presence, false, rayoHeaders);
                    sendPacket(presence);
                }
                else

                if ("ACTIVE".equals(origCallState) && "RING_WAIT".equals(callState) && "outbound".equals(callDirection))
                {
                    setRingingState(presence, false, rayoHeaders);
                    sendPacket(presence);
                }
                ...
                if (("RING_WAIT".equals(origCallState) || "RINGING".equals(origCallState)) && "ACTIVE".equals(callState) && "outbound".equals(callDirection))
                {
                    rayoHeaders.put("call_action", "join");
                    setAnsweredState(presence, false, rayoHeaders);
                    sendPacket(presence);
                ...
                if ("UNHELD".equals(callState))
                {
                    rayoHeaders.put("call_action", "join");
                    presence.getElement().add(rayoProvider.toXML(new AnsweredEvent(null, rayoHeaders)));
                    sendPacket(presence);
                }
